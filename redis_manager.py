...
class RedisManager:
    ...
    async def add_sms_mailing(
        ...
        async with self.redis.pipeline(transaction=True) as pipe:
            ...
            for phone in phones:
                ...
                await pipe.hset(mailing_phones_key, phone, "pending")
    ...
    async def get_sms_mailings(self, sms_ids: list[str]) -> list:
        """For each mailing in sms_ids load all data from Redis and return dict."""
        pipe = self.redis.pipeline()
        for sms_id in sms_ids:
            print(f"get_sms_mailings: {sms_id=}")
            sms_id_key = _clean_key(sms_id)
            mailing_key = f"sms_mailing_{sms_id_key}"
            phones_key = f"phones_for_sms_mailing_{sms_id_key}"

            await pipe.get(mailing_key)
            await pipe.hgetall(phones_key)
